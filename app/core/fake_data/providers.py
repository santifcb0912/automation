...
# Ruta al JSON con lista plana de nombres (misma pool para todos los paises)
NAMES_FILE = Path(__file__).resolve().parent.parent.parent / "config" / "data" / "names.json"

# Ruta al JSON con plantillas de telefonos con formato local por pais
...
class RandomNameProvider(INameProvider):
    ...
    # Retorna un nombre al azar de la pool global (independiente del pais)
...
class RandomPhoneProvider(IPhoneProvider):
    ...
    # Configura la ruta al JSON de telefonos. Usa el archivo por defecto si no se especifica otro
    def __init__(self, phones_file: Optional[Path] = None):
        self._phones_file = phones_file or PHONES_FILE
        self._templates: dict[str, str] = {}
        self._loaded = False

    # Retorna un telefono aleatorio del pais indicado. Si no hay template, retorna vacio
    ...
    # Carga las plantillas de telefonos desde el archivo JSON si no estan cargadas
    def _ensure_loaded(self):
        if self._loaded:
            return
    ...
    # Genera un numero telefonico aleatorio a partir de una plantilla de telefono
    @staticmethod

...
class FormFillerOrchestrator:
    ...
    # Formatea un timeout de Playwright como error para el lead
    def _timeout_error(self, detail: str, exc: PlaywrightTimeoutError) -> str:
        logger.error(f"Timeout {detail}: {self.page.url}")
    ...
    async def fill(self, lead: LeadRow) -> Optional[str]:
        ...
        try:
            ...
            return self._timeout_error("la pagina LP no cargo en 45s", e)
        try:
            await self._prepare_form(level)
        except PlaywrightTimeoutError as e:
            return self._timeout_error(f"el formulario {self._form_type} no se pudo preparar", e)
        try:
            scope = await self._find_scope()
        except PlaywrightTimeoutError as e:
            return self._timeout_error("no se detecto el scope del formulario", e)
        ...
        try:
            ...
            return self._timeout_error("el llenado del formulario excedio el tiempo", e)
    ...
    # Navega a la landing page, espera networkidle con timeout suave y pausa de estabilizacion
    ...
    # Espera networkidle con timeout suave. Si no llega, captura la excepcion y continua
    ...
    # Prepara el flujo de llenado segun el tipo de formulario
    ...
    # Tarjeta: si #TarjetaBLC existe en la LP, Plan A (fill directo). Si no, Plan B (buscar LP via ProgramSearchEngine)

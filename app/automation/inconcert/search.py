...
class InConcertSearch:
    ...
    # Constructor: inyecta page y la URL de contactos ya construida por InConcertClient
    ...
    # Orquesta: asegura contactos, recarga, pone filtro Email, escribe el correo y reintenta hasta 120s
    ...
    # Navegacion: si no esta en /contact/people, redirige a la pagina de contactos
    ...
    # Filtro: abre el dropdown de busqueda basica y selecciona la opcion "Email"
    async def _select_email_filter(self) -> bool:
        strategies = [
            ("ngb_dropdown", self.page.locator("button.btn-dropdown-text[ngbdropdowntoggle]").first),
            ("bootstrap_dropdown", self.page.locator("button.btn-dropdown-toggle[data-toggle='dropdown']").first),
    ...
    # Input: localiza el campo de texto por su placeholder y lo retorna
    ...
    # Reintentos: intenta hasta 8 veces (c/15s) buscar y verificar si el lead llego a InConcert
    ...
    # Busqueda: localiza el boton "Buscar" por su atributo title y hace click
    ...
    # Verificacion: revisa si ha llegado el lead en la tabla de resultados
    ...
    # Expande una seccion, verifica su contenido y ajusta tiempos si una seccion es mas lenta
    async def _expand_section(self, section_text: str, verification_text: str, *,
                              section_timeout: int = 10000, scroll_timeout: int = 3000,
                              delay_before: tuple = (200, 400), delay_after: tuple = (500, 800),
                              scroll_on_failure: bool = False) -> Optional[str]:
        ...
        try:
            ...
            verify = await self._find_verification(verification_text)
            ...
            if scroll_on_failure and section_clicked:
                await self._scroll_to_section_first_fields(section_text)
    ...
    # Localiza el texto de verificacion, reintentando con patron que tolera acentos
    async def _find_verification(self, verification_text: str):
    ...
    # Posiciona el viewport sobre los primeros campos de una seccion ya expandida
    async def _scroll_to_section_first_fields(self, section_text: str) -> None:
        containers = [
            self.page.locator("[class*='accordion-item']").filter(has_text=section_text).last,
            self.page.locator("[class*='card']").filter(has_text=section_text).last,
        ]
        for container in containers:
            if await container.count() == 0:
                continue
            try:
                content = container.locator("[role='region'], [class*='accordion-body'], [class*='card-body']").first
                await content.wait_for(state="attached", timeout=3000)
                field = content.locator("input, select, textarea, [class*='field']").first
                ...
                logger.debug(f"Scroll a seccion '{section_text}' fallo: {e}")
                continue
        header = self.page.get_by_text(section_text, exact=True).first

...
class ProgramSearchEngine:
    ...
    def for_country(country: str, page: Page, form_scope: Locator) -> 'ProgramSearchEngine':
        if country in ("argentina", "colombia", "peru", "usa", "bolivia", "chile", "paraguay", "dominicana", "guatemala", "panama", "el_salvador"):
            return LupaProgramSearchEngine(page, form_scope)
        return ProgramSearchEngine(page, form_scope)


# Paises con lupa: clickea icono si input no esta visible
class LupaProgramSearchEngine(ProgramSearchEngine):

    # Click lupa del navbar para revelar input de busqueda
    async def _locate_search_input(self):
        lupa = self.page.locator("div#navbar button[aria-label='abrir buscador']")
        if await lupa.count() == 0:
            lupa = self.page.locator("button[aria-label='abrir buscador']").first
        await lupa.click(timeout=5000)
        await self.page.wait_for_timeout(3000)
        return self.page.locator("input[placeholder*='Buscar']:not([placeholder*='carreras'])")

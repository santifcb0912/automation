...
- `Protocol` de typing → tipado estructural. Cualquier clase con `prepare()` y `fill()` que coincidan en firma es un `IFormFiller`, sin necesidad de herencia explícita.
- `prepare(form_type, level)` → prepara el formulario según su tipo (ej: abrir panel lateral con CTA o menú hamburguesa).
- `fill(ctx)` → recibe un `FillContext` con los datos del lead. Retorna `None` si tuvo éxito, o un `str` con la razón del error.
- Es la interfaz mínima posible (2 métodos) — cumple Interface Segregation Principle.
...
│  │  │ async def prepare(self,        │ │   │  strategy concreta:        │  │  
│  │  │     form_type: str,            │ │   │                            │  │  
│  │  │     level: str) -> None        │ │   │  • form_scope (Locator)    │  │  
│  │  │ async def fill(self,           │ │   │  • level (str)            │  │   
│  │  │     ctx: FillContext           │ │   │  • raw_level (str)        │  │   
│  │  │ ) -> Optional[str]             │ │   │  • test_email (str)       │  │   
│  │  └─────────────────────────────────┘ │   │  • fake_name (str)        │  │  
│  │                                      │   │  • fake_phone (str)       │  │  
│  │  None  = éxito                       │   │                            │  │ 
│  │  str   = razón del error             │   │                            │  │ 
...
│    │      return CmsFiller(...)   raw_level=...,           │                │ 
│    │                             test_email=...,          │                │  
│    │                             fake_name=...,           │                │  
│    │  fillers/ implementan        fake_phone=...,          │                │ 
│    │  IFormFiller:               )                         │                │ 
│    │                             error = await            │                │  
│    │  class CmsFiller(                filler.fill(ctx)    │                │  
│    │    IFormFiller):                                     │                │  
│    └──────────────────────────────────────────────────────┘                │  
│                                                                              │
└──────────────────────────────────────────────────────────────────────────────┘
...
│       │                           ↓                                         │ 
│       │               registry devuelve CmsFiller o FallbackFiller           │
│       │               (ambos implementan IFormFiller)                        │
│       │                                                                      │
...
│      error = await CmsFiller(config, page, country, fake)                    │

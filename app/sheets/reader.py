...
class SheetsReader:
    ...
    # Establece la conexion con Google Sheets usando la Service Account
    ...
    # Lee el Sheets y retorna los leads del pais indicado para la hoja especificada
    def get_leads(
        self,
        country_name: str,
        sheet_id: str,
        ...
        for row_idx, row in enumerate(all_rows[1:], start=2):
            lead = self._parse_row(row, row_idx, country_name)
            if lead is not None:
                leads.append(lead)
    ...
    # Convierte una fila del Sheets en un LeadRow si el pais y la URL coinciden
    def _parse_row(self, row: List[str], row_idx: int, country_name: str) -> Optional[LeadRow]:
        if len(row) < 5:
            return None
        ...
        if not country_val:
            return None
        # Normaliza acentos: "Perú" → "Peru", "México" → "Mexico"
        norm_val = unicodedata.normalize('NFKD', country_val).encode('ascii', 'ignore').decode('ascii').lower()
        norm_target = country_name.lower()
        # Matchea en ambas direcciones: "El Salvador" con celda "Salvador" y viceversa
        if norm_target not in norm_val and norm_val not in norm_target:
            return None
    ...
    # Normaliza el valor de tipo de formulario usando un mapa de sinonimos
    @staticmethod
    def _normalize_form_type(raw: str) -> str:
        key = raw.lower().replace(" ", "").replace("-", "").replace("_", "")
        return FORM_TYPE_MAP.get(key, raw)

    # Retorna la letra de la columna del Sheets correspondiente al dia de hoy
